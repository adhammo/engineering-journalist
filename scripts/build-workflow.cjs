...
node('Research','chainLlm',{promptType:'define',text:'={{ $json.prompt }}',messages:{messageValues:[]},batching:{}},1680,0,1.9);
nodes[nodes.length-1].type='@n8n/n8n-nodes-langchain.chainLlm';
...
node('Human Evidence Review','wait',{
 ...
 formFields:{values:[{fieldLabel:'Reviewer',fieldType:'text',requiredField:true},...Array.from({length:8},(_,i)=>({fieldLabel:'E'+String(i+1).padStart(2,'0'),fieldType:'dropdown',fieldOptions:{values:['Read','Ignore','Investigate'].map(option=>({option}))},requiredField:`={{ $('Render Draft Dashboard').first().json.items.length > ${i} }}`})),{fieldLabel:'Source verification',fieldType:'dropdown',fieldOptions:{values:[{option:'Confirmed'},{option:'Needs investigation'}]},requiredField:true},{fieldLabel:'Review notes',fieldType:'textarea',requiredField:false}]},options:{}
