...
node('Human Evidence Review','wait',{
 ...
 formDescription:"={{ 'Open this evidence snapshot: ' + $('Render Draft Dashboard').first().json.reviewPagesUrl + '\nGitHub file: ' + $('Render Draft Dashboard').first().json.reviewGithubUrl + '\nVerify original sources. Read = include, Ignore = exclude, Investigate = hold. There are ' + $('Render Draft Dashboard').first().json.items.length + ' items. Leave unused ID slots blank. Zero items: submit your name to acknowledge coverage.' }}",
...
node('Human Publication Approval','wait',{
 ...
 formDescription:`={{ 'Review the immutable draft: ' + ${run}.draftPagesUrl + '\nPinned GitHub file: https://github.com/' + ${run}.owner + '/' + ${run}.repo + '/blob/' + $('Archive Reviewed Draft').first().json.body.commit.sha + '/' + ${run}.draftPath + '\nApprove publishes these exact bytes to index.html. Reject preserves the previous dashboard. The root review.html is a convenience pointer only.' }}`,
...
const chain=['Manual Trigger','Repository Settings','Resolve Source Revision','Fetch Config','Fetch Prompt Template','Fetch Dashboard Template','Build Research Prompt','Gemini Research','Parse Research JSON','Render Draft Dashboard','Archive Evidence Snapshot','Human Evidence Review','Apply Human Decisions','Render Reviewed Dashboard','Archive Evidence and Decisions','Archive Reviewed Draft','Get Current Review','Prepare Review Commit','Publish for Review','Human Publication Approval','Record Publication Decision','Save Publication Decision','Approved?','Fetch Approved Review','Get Current Index','Prepare Publish Commit','Publish Approved Dashboard','Result'];
