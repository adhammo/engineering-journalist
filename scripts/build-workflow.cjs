...
const sourceFiles=['repository-settings.js','build-research-prompt.js','check-research-response.js','parse-research-json.js','render-report.js','render-draft-dashboard.js','apply-human-decisions.js','render-reviewed-dashboard.js','prepare-review-commit.js','record-publication-decision.js','prepare-publish-commit.js','publication-result.js'];
...
http('Gemini Research',"={{ 'https://generativelanguage.googleapis.com/v1beta/models/' + $json.config.gemini_model + ':generateContent' }}",1680,0,{method:'POST',body:'={{ $json.body }}',credential:'googlePalmApi'});
code('Check Research Response','check-research-response.js',1680,-200);
node('Retry Empty Answer?','if',{conditions:{options:{typeValidation:'strict',version:2},conditions:[{leftValue:'={{ $json.retryEmptyAnswer }}',rightValue:true,operator:{type:'boolean',operation:'true',singleValue:true}}],combinator:'and'},options:{}},1920,-200,2.2);
http('Retry Empty Gemini Response',"={{ 'https://generativelanguage.googleapis.com/v1beta/models/' + $('Build Research Prompt').first().json.config.gemini_model + ':generateContent' }}",2160,-200,{method:'POST',body:"={{ $('Build Research Prompt').first().json.body }}",credential:'googlePalmApi'});
...
node('Human Evidence Review','wait',{
 ...
 formDescription:"={{ 'Open this evidence snapshot: ' + $('Render Draft Dashboard').first().json.reviewPagesUrl + '<br>GitHub file: ' + $('Render Draft Dashboard').first().json.reviewGithubUrl + '<br>Verify original sources. Read = include, Ignore = exclude, Investigate = hold. There are ' + $('Render Draft Dashboard').first().json.items.length + ' items. Leave unused ID slots blank. Zero items: submit your name to acknowledge coverage.' }}",
 formFields:{values:[{fieldLabel:'Reviewer',fieldType:'text',requiredField:true},...Array.from({length:8},(_,i)=>({fieldLabel:'E'+String(i+1).padStart(2,'0'),fieldType:'dropdown',fieldOptions:{values:['Read','Ignore','Investigate'].map(option=>({option}))},requiredField:`={{ $('Render Draft Dashboard').first().json.items.length > ${i} }}`})),{fieldLabel:'Review notes',fieldType:'textarea',requiredField:false}]},options:{}
},2400,400,1.1,{webhookId:id('human-evidence-review'),notes:'Open the waiting execution resume-form URL in n8n. No decision defaults or approval timeout. Eight slots; only existing items require decisions.',notesInFlow:true});
...
const chain=['Manual Trigger','Repository Settings','Resolve Source Revision','Fetch Config','Fetch Prompt Template','Fetch Dashboard Template','Build Research Prompt','Gemini Research','Check Research Response','Retry Empty Answer?','Retry Empty Gemini Response','Parse Research JSON','Render Draft Dashboard','Archive Evidence Snapshot','Human Evidence Review','Apply Human Decisions','Render Reviewed Dashboard','Archive Evidence and Decisions','Archive Reviewed Draft','Get Current Review','Prepare Review Commit','Publish for Review','Human Publication Approval','Record Publication Decision','Save Publication Decision','Approved?','Fetch Approved Review','Get Current Index','Prepare Publish Commit','Publish Approved Dashboard','Result'];
